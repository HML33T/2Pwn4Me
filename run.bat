...
::Unmount x: drive, as we'll need to mount it to SMB and we can't if it's premounted.
if %errorlevel% == 0 ( 
		echo Succesful!
	) else (
...
if %errorlevel% == 0 ( 
		::Check if it was succesful as it is needed!
...
::Copying files no aborts here as it could be because of a overwrite issue, which is non critical
...
::Audio player dll files
if %errorlevel% == 0 (echo Copy succesful) else (echo Failed to copy file!)
cd plugins
echo Stage 2 of 3 (3/3)
copy *.* X:\rc\plugins\*.* >nul 2>&1 
::More audio player dll files
if %errorlevel% == 0 (echo Copy succesful) else (echo Failed to copy file!)
cd..
echo Stage 3 of 3
...
if %errorlevel% == 0 (echo Copy succesful) else (echo Failed to copy file!)
echo Complete! 
::This prosses shouldn't take longer than 10 seconds, unless your song is really long!
echo.
